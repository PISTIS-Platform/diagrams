...
sequenceDiagram
    ...
    User ->> dataCheckIn: Provide File's Name/Path and Metadata, and Request to Check-In
    dataCheckIn ->> dataSource: Get File
    dataSource ->> dataCheckIn: File
    dataCheckIn ->> factoryDataStorage: Store Data File
    factoryDataStorage ->> dataCheckIn: Return UUID
    dataCheckIn ->> factoryDataCatalogue: Store Metadata (UUID)
    factoryDataCatalogue ->> dataCheckIn: Acknowledge
    dataCheckIn ->> User: Check-In Confirmation

    opt Data Processing 
        User ->> jobConfigurator: Configure Pipeline
        jobConfigurator ->> User: Acknowledge
        User ->> jobConfigurator: Trigger Pipeline
        jobConfigurator ->> factoryDataStorage: Get Data File
        factoryDataStorage ->> jobConfigurator: Return Data File
        opt Transformation
            jobConfigurator ->> dataTransformation: Transform Data in File
            dataTransformation ->> jobConfigurator: Return Data File
        end
        opt Insights
